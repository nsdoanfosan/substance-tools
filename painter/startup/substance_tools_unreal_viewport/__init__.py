...
def _save_successful_request():
    ...
    try:
        ...
        for key in ("pipeline_hash", "low_hash", "high_hash", "settings_hash"):
...
def _on_project_ready(_event=None):
    ...
    metadata = substance_painter.project.Metadata(METADATA_CONTEXT)
    if metadata.get("pipeline_hash") == request.get("pipeline_hash"):
...
def start_plugin():
    ...
    _pending_timer.timeout.connect(_create_pending_project)
